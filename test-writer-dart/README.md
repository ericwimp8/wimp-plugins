...
Test generation for Dart and Flutter using a multi-agent workflow.
...
Available skills for Dart:
- **dart-testing** — Dart/Flutter testing with Mocktail: mocks vs fakes, stubbing, verification, async patterns
- **riverpod-testing** — Riverpod 3.0 testing patterns: ProviderContainer.test(), mocking providers, testing AsyncNotifier

---

## Commands

### /write-tests-dart `<source-file>`

Orchestrates comprehensive test writing for a Dart source file.
...
### /fix-tests-dart `<source-file>`

Runs existing tests and fixes failures for a Dart source file.

**Process:** Discovers failing tests, then invokes **tw-run-fix** for each failure with iteration and retry loops.

---

### /fix-all-tests-dart `<source-file>...`
