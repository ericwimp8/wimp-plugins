---
description: Orchestrate comprehensive test writing for a Dart source file
...
Invoke the `test-writer-dart:tw-plan-cases` agent using the prompt template below.
...
Invoke the `test-writer-dart:tw-check-redundancy` agent using the prompt template below.
...
Invoke the `test-writer-dart:tw-write-test` agent using the prompt template below.
...
Example: `test/reports/user_service_report_2024-01-07_143022.md`
...
**Plan file to remove**: `test/plans/[filename]_plan.md`

Use the Bash tool to delete the file:
```
rm test/plans/[filename]_plan.md
```
